...
def format_excerpt_block(excerpt: str) -> str:
    if not excerpt:
        return "excerpt: ''"
...
def gather_posts(directory: Path):
    ...
    return sorted(posts, key=lambda entry: slug_date(entry['date']), reverse=True)


def build_manifest():
    if not ROOT.exists():
        print(f'Posts directory {ROOT} is missing', file=sys.stderr)
        sys.exit(1)
    ...
    for subdir in sorted(p for p in ROOT.iterdir() if p.is_dir() and p.name != 'default'):
        category_posts = gather_posts(subdir)
